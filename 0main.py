...
def get_movie_details(genre_ids, genre_names):
    for i in range(len(genre_ids)):
        ...
        if not os.path.exists("posters2"):
            os.makedirs("posters2")

        # create a folder for each genre inside the 'posters' folder
        if not os.path.exists(f"posters2/{genre_names[i]}"):
            os.makedirs(f"posters2/{genre_names[i]}")
        ...
        if not os.path.exists("movielists2"):
            os.makedirs("movielists2")

        print(f"Downloading posters and writing movie list for {genre_names[i]} with {len(movielist)} movies")

        for movie in movielist:
            with open(f"movielists2/{genre_names[i]}.out", "a") as file:
...
def get_movielist(genre_id, genre_name):
    ...
    page_number = 1
    while counter < 2000:
        url = f"https://api.themoviedb.org/3/discover/movie?include_adult=true&include_video=false&page={str(page_number)}&sort_by=primary_release_date.desc&sort_by=vote_count.desc&sort_by=popularity.desc&with_genres={genre_id}"
...
def download_poster(poster_path, movie_id, movie_title, genre_name, counter, retry_count):
    ...
    if response.status_code == 200:
        with open(f"posters2/{genre_name}/{counter}_{movie_id}.jpg", "wb") as file:
